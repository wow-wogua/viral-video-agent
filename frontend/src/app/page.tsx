...
const templates = [
  { title: 'B站赛道爆款分析', description: '找到代表样本，归纳内容形态与竞争结构。', kicker: 'Track scan', icon: BarChart3, span: 'md:col-span-2' },
  { title: '竞品账号内容拆解', description: '围绕指定账号或竞品需求提取可复用策略。', kicker: 'Competitor', icon: Target, span: '' },
  { title: '热门标题结构分析', description: '对标题钩子、信息密度与承诺方式做证据化拆解。', kicker: 'Headline', icon: SearchCheck, span: '' },
  { title: '选题方向分析', description: '从真实样本与知识库中筛选值得验证的方向。', kicker: 'Topic map', icon: FileSearch, span: '' },
  { title: '内容脚本深度分析', description: '在转写能力可用时分析开头、口播与脚本结构。', kicker: 'Transcript', icon: Link2, span: 'md:col-span-2 lg:col-span-1' },
];

const evidenceFeatures = [
  ['稳定引用', '每条证据获得可校验的 evidence_id。'],
  ['结论门禁', '不存在的引用会使报告验证失败。'],
  ['来源可回看', '点击引用即可打开 Evidence 与真实来源。'],
  ['边界透明', '不会把单个视频外推为整个行业规律。'],
];
  ...
  return (
    <main>
        ...
        <div className="mx-auto max-w-7xl px-4 pb-16 pt-8 sm:px-6 sm:pb-20 lg:px-8 lg:pb-24">
          ...
          <div className="grid gap-10 pt-12 lg:grid-cols-[minmax(0,1.05fr)_minmax(25rem,.8fr)] lg:items-center lg:gap-16 lg:pt-16">
            <div className="motion-enter min-w-0">
              ...
              <p className="mt-7 max-w-2xl text-pretty text-lg leading-8 text-muted-foreground">输入赛道、选题或竞品需求，生成带来源链接、结构化结论和可执行建议的 B 站内容分析报告。每条关键观察都能回到 Evidence。</p>
              <div className="mt-8 flex flex-wrap gap-3"><LinkButton href="/register" size="lg">开始公开测试<ArrowRight className="h-4 w-4" aria-hidden="true" /></LinkButton><LinkButton href="/examples" size="lg" variant="secondary">查看示例报告</LinkButton></div>
              <div className="mt-8 grid max-w-2xl grid-cols-3 border-y py-4">
                {[['真实 URL', '来源'], ['结构化 Claim', '结论'], ['B站单平台', '边界']].map(([value, label], index) => <div key={label} className="border-r px-3 first:pl-0 last:border-r-0 last:pr-0"><p className="font-mono text-xs font-semibold text-foreground sm:text-sm">{value}</p><p className="mt-1 text-[11px] text-muted-foreground">0{index + 1} / {label}</p></div>)}
              </div>
            </div>
            ...
            <Card className="motion-enter-delay relative overflow-hidden bg-surface-elevated p-0 shadow-float">
              <div className="flex items-center justify-between gap-4 bg-secondary px-5 py-4 text-secondary-foreground">
                <div><p className="font-mono text-[10px] uppercase tracking-[.16em] text-primary">Report pipeline</p><p className="mt-1 text-sm font-bold">AI 编程赛道的爆款选题与标题结构</p></div>
              ...
              <div className="relative p-5 sm:p-6">
                ...
                <div className="relative space-y-0">
                  {['采集 8 条真实 B 站视频样本', '检索 5 条知识库依据', '验证 12 条结构化结论', '生成策略报告与数据附录'].map((item, index) => (
                ...
                <div className="relative mt-5 border-l-4 border-primary bg-primary/10 p-4 text-sm leading-6"><span className="font-bold">结论示例：</span>高播放样本常用“结果前置 + 明确受众”的标题结构。<Link href="#evidence" className="ml-2 font-mono text-xs font-semibold text-primary underline underline-offset-4">[ev_bili_01]</Link></div>
              </div>
            </Card>
          </div>
        </div>
      ...
      <section id="templates" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
        <div className="grid gap-6 border-b pb-8 lg:grid-cols-[.55fr_1fr] lg:items-end">
          <div><p className="eyebrow">Analysis formats / 02</p><h2 className="title-balance mt-3 text-4xl font-black tracking-[-.045em] sm:text-5xl">从真实问题开场</h2></div>
          <div><p className="max-w-2xl text-pretty text-base leading-7 text-muted-foreground">选择模板后仍可自由编辑需求。产品不会展示尚未接入的其他平台入口，也不会把 Agent 编排当成替代业务结果的卖点。</p></div>
        </div>
        <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {templates.map(({ title, description, kicker, icon: Icon, span }, index) => (
            <Card key={title} className={`group relative min-h-56 overflow-hidden p-5 transition-[border-color,box-shadow] hover:border-primary/40 hover:shadow-float sm:p-6 ${span}`}>
              ...
              <p className="eyebrow mt-7">{kicker}</p><h3 className="mt-2 text-xl font-bold tracking-[-.025em]">{title}</h3><p className="mt-3 max-w-md text-pretty text-sm leading-6 text-muted-foreground">{description}</p>
              <div className="mt-6 flex items-center gap-2 text-xs font-semibold text-primary opacity-70 transition-opacity group-hover:opacity-100">查看分析入口<ArrowRight className="h-3.5 w-3.5" aria-hidden="true" /></div>
            </Card>
          ))}
        </div>
      </section>
      ...
      <section id="evidence" className="relative overflow-hidden bg-secondary text-secondary-foreground">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-12 px-4 py-16 sm:px-6 sm:py-20 lg:grid-cols-[.8fr_1.2fr] lg:px-8 lg:py-24">
          <div>
            <p className="eyebrow">Evidence system / 03</p><h2 className="title-balance mt-4 text-4xl font-black tracking-[-.045em] sm:text-5xl">数字与来源，<br />必须一一对应。</h2>
            <p className="mt-6 max-w-xl text-pretty leading-8 text-secondary-foreground/68">系统保存真实 URL、抓取时间、BVID、作者和可用数据字段。Observation、Inference 与 Recommendation 分层展示，证据不足时明确降级为部分结果。</p>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
        <div className="grid gap-10 lg:grid-cols-[.7fr_1.3fr]">
          <div><p className="eyebrow">Workflow / 04</p><h2 className="title-balance mt-3 text-4xl font-black tracking-[-.045em]">四步完成一次内容研究</h2><p className="mt-4 text-pretty leading-7 text-muted-foreground">公开测试使用可靠轮询呈现后台任务进度，页面可安全关闭和恢复。</p></div>
          <ol className="grid border-t sm:grid-cols-2">
            {['写清赛道、选题或竞品需求', '采集公开样本并检索知识库', '验证结构化结论与 Evidence', '生成报告、附录与分享链接'].map((item, index) => <li key={item} className="grid min-h-32 grid-cols-[3rem_1fr] gap-3 border-b p-4 sm:border-r sm:p-5"><span className="font-mono text-2xl font-black text-primary">0{index + 1}</span><span className="text-sm font-semibold leading-6">{item}</span></li>)}
          </ol>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 pb-16 sm:px-6 sm:pb-20 lg:px-8 lg:pb-24">
        <div className="relative overflow-hidden bg-primary p-7 text-primary-foreground shadow-float sm:p-9 lg:flex lg:items-center lg:justify-between lg:gap-10">
          ...
          <div className="relative"><p className="font-mono text-[10px] font-semibold uppercase tracking-[.16em]">Open beta / Bilibili only</p><h2 className="title-balance mt-2 text-3xl font-black tracking-[-.04em]">准备好测试一个真实需求了吗？</h2><p className="mt-2 text-sm text-primary-foreground/80">小规模公开测试期间，所有分析都限定在 B 站。</p></div>
          <LinkButton href="/register" variant="secondary" size="lg" className="relative mt-6 border-white/50 bg-white text-secondary hover:bg-white/90 lg:mt-0">创建账号<ArrowRight className="h-4 w-4" aria-hidden="true" /></LinkButton>
        </div>
      </section>
    </main>
  );
