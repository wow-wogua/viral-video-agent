...
- [P0-C 分层架构失败实验与冻结说明](docs/content-intelligence-p0c-architecture-repair.md)
...
当前阶段总控状态：P0-A 带保留通过，P0-B 工程 Gate 通过；P0-C 在 v3 新盲评和一次额外授权的分层架构 development 实验均失败后正式冻结；P0-D、P0-E 均未开始。P0 整体因 P0-C 质量 Gate 失败而终止，不是已经完成。

当前完整 Python 回归基线为 191 条测试。P0-C 的工程链路、Creator Provider Recovery 与 UAPI development-only 接入已实现。UAPI 完整采集取得 387/394 可评分 Creator 样本和 98.22% 可用覆盖，解决了原数据源阻塞；但原冻结 v1 Gate 仍因 selected precision 18.42%、strict Precision@5 33.33% 和 unresolved selection rate 76.32% 未通过。不相关误判率为 5.26%，来源追溯和分数拆解通过。
...
P0-C v3 在不覆盖 v1/v2 的前提下新增 `creator-topic-assessment.p0.2`、`creator-qualification.p0.2`、`competitor-selection.p0.3` 和独立的 `competitor-evaluation.p0.2` truth mapping。53 项 development 校准不是最终 Gate；随后完成的新盲评 selected precision 为 57.14%、strict Precision@5 为 80.00%、不相关误判率为 14.29%，且 14/20 关键词零输出，因此 Gate 为 `failed`。

总控随后只额外授权了一次真正分层的架构 development 实验。该实验只在 97 项 development set 上评估：relevance exact 提高到 71.13%、role exact 提高到 43.30%，但 specialization exact 退化到 55.67%、qualification exact 退化到 43.30%；selected precision 69.23%、strict Precision@5 50.00%、不相关误判率 7.69%、output coverage 66.67%，只选出 13 项且 14/20 关键词 abstain。它不是新 holdout 或线上指标，没有 architecture freeze，也没有把失败候选采用为运行时代码；实验源码只保存在本地归档分支和仓库外 Bundle，未推送 GitHub。完整边界见 [P0-C v3 记录](docs/content-intelligence-p0c-v3.md)和[分层架构失败实验说明](docs/content-intelligence-p0c-architecture-repair.md)。
