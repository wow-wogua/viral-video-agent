# P0-C v3：账号主题资格校准、失败盲评与阶段冻结

历史冻结候选版本：`creator-topic-assessment.p0.2` / `creator-qualification.p0.2` / `competitor-selection.p0.3` / `competitor-evaluation.p0.2`

## 阶段边界

P0-C v3 只修复账号—主题预测、资格门禁、核心账号排序和独立评测语义。它不进入 P0-D，不新增 Creator/UAPI/B站/网络请求，不调整冻结关键词，不生成代表视频、ASR、商业指标或完整情报报告，也不新增 migration。

53 项人工审核只作为 development/error-analysis set。新盲评从未进入这 53 项的账号—关键词关系中生成；代码、规则和选择结果先提交冻结，盲评标签只能用于 `competitor-evaluation.p0.2` truth mapping 和 Gate 指标。该盲评现已完成并失败，本文件保留其设计与最终结论，不再描述待执行阶段。
...
代码提交冻结后，从全量冻结关系中先排除 53 项 development set，再运行同一 v3 资格和排序。实际盲评包遵守以下边界：
...
冻结结果为 selected precision 57.14%、strict Precision@5 80.00%、irrelevant false-positive rate 14.29%，且 14/20 关键词零输出。selected precision 与不相关误判率均未达标，输出覆盖不足，因此 v3 Gate 为 `failed`，不能承诺 Top 5 质量已经达标。
...
v3 新盲评失败后，总控只额外授权了一次真正分层的架构 development 实验。该实验只使用 97 项 development set，没有生成第二轮 holdout：relevance exact 提高到 71.13%、role exact 提高到 43.30%，但 specialization exact 退化到 55.67%、qualification exact 退化到 43.30%；selected precision 69.23%、strict Precision@5 50.00%、不相关误判率 7.69%、output coverage 66.67%，selected count 为 13，14/20 关键词 abstain。

这些只是 development 结果，不是新 holdout 或最终线上指标。架构候选状态为 `architecture_candidate_invalid_stop`，没有 architecture freeze，没有采用为运行时代码，也没有继续修改算法、Prompt、资格或排序。失败源码只保存在本地归档分支和仓库外 Bundle，未推送 GitHub。

最终阶段状态：P0-A 带保留通过，P0-B 工程 Gate 通过，P0-C 正式冻结；P0-D、P0-E 均未开始。P0 因 P0-C 质量 Gate 失败而终止，不承诺 Top 5 质量已达标、全站覆盖、市场份额、趋势或生产 SLA。
