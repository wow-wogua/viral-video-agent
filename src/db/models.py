...
class CrawlRun(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    job_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("analysis_jobs.id", ondelete="CASCADE"), nullable=True, index=True)
    schema_version: Mapped[str] = mapped_column(String(40))
    snapshot_revision: Mapped[str | None] = mapped_column(String(32), nullable=True)
...
class CrawlRunVideo(Base):
    ...
    aid: Mapped[int | None] = mapped_column(Integer, nullable=True)
    creator_mid: Mapped[str | None] = mapped_column(String(32), nullable=True, index=True)
    creator_name: Mapped[str | None] = mapped_column(Text, nullable=True)
    title: Mapped[str] = mapped_column(Text)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    tags: Mapped[list[str]] = mapped_column(JSON, default=list)
    ...
    relevance_labeler: Mapped[str | None] = mapped_column(String(40), nullable=True)
    relevance_labeler_version: Mapped[str | None] = mapped_column(String(80), nullable=True)
    raw_payload_hash: Mapped[str | None] = mapped_column(String(64), nullable=True)


class CrawlRunCreatorObservation(Base):
    __tablename__ = "crawl_run_creators"
    crawl_run_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("crawl_runs.id", ondelete="CASCADE"), primary_key=True)
    mid: Mapped[str] = mapped_column(String(32), primary_key=True, index=True)
    name: Mapped[str] = mapped_column(Text)
    profile_url: Mapped[str | None] = mapped_column(Text, nullable=True)
    avatar_url: Mapped[str | None] = mapped_column(Text, nullable=True)
    follower_count: Mapped[int | None] = mapped_column(Integer, nullable=True)
...
class CreatorAuditRecord(Base):
    __tablename__ = "creator_audits"
    __table_args__ = (UniqueConstraint("crawl_run_id", "creator_mid", name="uq_creator_audit_run_mid"),)
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    crawl_run_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("crawl_runs.id", ondelete="CASCADE"), index=True)
    ...
    source_provider_name: Mapped[str] = mapped_column(String(80))
    source_provider_version: Mapped[str] = mapped_column(String(80))
    source_url: Mapped[str] = mapped_column(Text)
    follower_count: Mapped[int | None] = mapped_column(Integer, nullable=True)
    sample_count: Mapped[int] = mapped_column(Integer, default=0)
    recent_30d_upload_count: Mapped[int] = mapped_column(Integer, default=0)
    recent_90d_upload_count: Mapped[int] = mapped_column(Integer, default=0)
    qualification_status: Mapped[str] = mapped_column(String(32))
    generalist: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
...
class CreatorSampleVideoRecord(Base):
    __tablename__ = "creator_sample_videos"
    __table_args__ = (
        UniqueConstraint("crawl_run_id", "creator_mid", "bvid", name="uq_creator_sample_video"),
    )
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    crawl_run_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("crawl_runs.id", ondelete="CASCADE"), index=True)
    creator_mid: Mapped[str] = mapped_column(ForeignKey("creators.mid", ondelete="CASCADE"), index=True)
    bvid: Mapped[str] = mapped_column(ForeignKey("videos.bvid", ondelete="CASCADE"), index=True)
    sample_rank: Mapped[int] = mapped_column(Integer)
    creator_name: Mapped[str] = mapped_column(Text)
    title: Mapped[str] = mapped_column(Text)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    tags: Mapped[list[str]] = mapped_column(JSON, default=list)
    ...
    missing_fields: Mapped[list[str]] = mapped_column(JSON, default=list)
    raw_payload_hash: Mapped[str | None] = mapped_column(String(64), nullable=True)
    relevance_label: Mapped[str] = mapped_column(String(20))
    relevance_reason: Mapped[str] = mapped_column(Text)
    relevance_confidence: Mapped[float] = mapped_column(Float)
    relevance_evidence_ids: Mapped[list[str]] = mapped_column(JSON, default=list)
    relevance_labeler: Mapped[str] = mapped_column(String(40))
    relevance_labeler_version: Mapped[str] = mapped_column(String(80))
...
class CompetitorScoreRecord(Base):
    ...
    evidence_ids: Mapped[list[str]] = mapped_column(JSON, default=list)
    search_candidate_sources: Mapped[list[str]] = mapped_column(JSON, default=list)
    creator_sample_sources: Mapped[list[str]] = mapped_column(JSON, default=list)
    tie_break_values: Mapped[list] = mapped_column(JSON, default=list)
